...
pub async fn run(name: &str, path: Option<&str>, adopt: bool) -> i32 {
    let identity = match crate::identity::load() {
        ...
        Ok(None) => {
            ...
            eprintln!(
                "  → Run 'kith init <name>' and give kith a name to attach to what you add."
            );
            return EX_CONFIG;
        }
        Err(e) => {
    ...
    let credentials = match credentials(&config) {
        ...
        Err(_) => {
            ...
            eprintln!("  → Install and start the daemon (Syncthing), then run kith doctor.");
...
/// What `kith create` did, in enough detail to say it out loud.
struct Made {
    circle: CircleRef,
    root: PathBuf,
...
fn resolve_root(
    ...
    match requested {
        Some(root) => {
            let replicated = known.iter().find(|c| same_path(&c.root, &root)).cloned();
            Ok((root, replicated))
        }
        None => {
...
fn default_root(name: &str) -> Result<PathBuf, Fault> {
    let home = directories::BaseDirs::new()
        ...
        .ok_or(Fault::NoHome)?;
    Ok(home.join("kith").join(slug(name)))
}
    ...
    fn report(&self) {
        match &self.content {
            ...
            Some(found) => {
                ...
                if self.kept_space {
                    println!(
                        "  circle      {} ({}) — kith kept the synced space it was already in; nothing moved",
                ...
                if found.unreadable > 0 {
                    println!("  unreadable  {} files kith could not read", found.unreadable);
                }
            }
        }
        ...
        match &self.stewardship {
            ...
            Stewardship::Awaiting(device) => {
                println!(
                    "This Circle has no kith record yet; kith cannot name its Steward until {} runs kith.",
                    fingerprint(device)
                );
    ...
    fn report(&self, address: &str) {
        match self {
            Fault::BadName(why) => {
                ...
                eprintln!("  → Give it 1–64 printable characters: kith create walls");
            }
            Fault::NoHome => {
                eprintln!("✗ kith cannot work out where this Person's home directory is.");
                eprintln!("  → Say where the Circle should live: kith create <name> --path <dir>");
            }
            ...
            Fault::Engine(SyncError::Unauthorized) => {
                ...
                eprintln!(
                    "  → Check its API key. kith never rewrites the daemon's configuration; run kith doctor to see where the key was read from."
                );
            }
            Fault::Engine(SyncError::Incompatible(version)) => {
                eprintln!("✗ The Sync Engine at {address} is below the version kith needs ({version}).");
                eprintln!("  → Upgrade the daemon, then run kith doctor.");
            }
            Fault::Engine(e) => {
            ...
            Fault::PathNotEmpty(root) => {
                eprintln!("✗ {} is not empty, and kith did not put anything there.", pretty(root));
                eprintln!(
            ...
            Fault::AdoptNotADirectory(root) => {
                ...
                eprintln!("  → Point --path at the wallpaper directory you want kith to take over.");
            }
            Fault::AdoptNotFound => {
                eprintln!("✗ kith found no existing synced wallpaper directory to adopt.");
                eprintln!("  → Name it: kith create <name> --adopt --path <dir>");
            }
            Fault::AdoptAmbiguous(candidates) => {
                eprintln!("✗ kith found more than one synced directory it could adopt:");
                for c in candidates {
                    eprintln!("    {}  {}", c.id.0, pretty(&c.root));
                }
                eprintln!("  → Say which: kith create <name> --adopt --path <dir>");
            }
            Fault::Io(path, e) => {
...
mod tests {
    ...
    impl SyncEngine for FakeEngine {
        ...
        async fn create_circle(&self, name: &str, root: &Path) -> Result<CircleRef, SyncError> {
            *self.creates.lock().unwrap() += 1;
            let made = CircleRef {
                id: CircleId("kith-4tj2q9xa".into()),
                name: name.to_string(),
                root: root.to_path_buf(),
            };
    ...
    async fn re_running_finishes_a_half_made_circle_without_making_a_second_one() {
        ...
        let engine = FakeEngine::new().replicating("kith-4tj2q9xa", "walls", &root);
        let id = ana();
    ...
    async fn a_directory_with_someone_elses_content_is_refused_rather_than_moved_into() {
        ...
        let fault = create(&engine, &ana(), "walls", Some(root.to_str().unwrap()), false)
            .await
            .err()
            .expect("kith does not move into an occupied directory");

        assert!(matches!(fault, Fault::PathNotEmpty(_)));
        assert_eq!(fault.exit(), EX_USAGE);
        assert!(!root.join(".kith").exists(), "nothing was written");
    ...
    async fn a_circle_by_that_name_already_here_is_refused_and_not_renamed() {
        ...
        let engine = FakeEngine::new().replicating("kith-aaaaaaaa", "Walls", &base.join("other"));
    ...
    async fn an_unreachable_engine_refuses_before_anything_is_written() {
        ...
        let fault = create(&engine, &ana(), "walls", Some(root.to_str().unwrap()), false)
            .await
            .err()
            .expect("creating a Circle writes engine config; kith will not queue that");
    ...
    async fn a_failed_descriptor_write_undoes_the_space_it_just_created() {
        ...
        assert_eq!(
            ...
            ["kith-4tj2q9xa"],
            "the space kith allocated is given back"
        );
        assert!(root.exists(), "an adopted root is never removed — kith did not create it");
        fs::remove_dir_all(&base).unwrap();
    }
    ...
    async fn adoption_never_touches_engine_artefacts_dot_entries_symlinks_or_unclaimed_content() {
        ...
        assert!(root.join("notes.txt").is_file(), "kith deletes nothing it did not create");
        fs::remove_dir_all(&base).unwrap();
    }
    ...
    async fn adopting_into_an_existing_descriptor_leaves_it_exactly_as_found() {
        ...
        let engine = FakeEngine::new().replicating("kith-bbbbbbbb", "walls", &root);
    ...
    async fn adopt_with_several_candidates_asks_which_one() {
        ...
        let fault = create(&engine, &ana(), "walls", None, true)
            .await
            .err()
            .expect("kith does not guess which tree a Person meant");
